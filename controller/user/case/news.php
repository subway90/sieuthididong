...
#RENDER VIEW
//FILTER BY CATEGORY
$showFilterCategoryNews = '<a href="'.URL.'tin-tuc/" class="btn btn-sm btn-outline-success me-1 '.matchValue('active',-1,$activeCate).'"><small>Tất cả</small></a>';
for ($i=0; $i < count($listCate); $i++) { 
    extract($listCate[$i]);
    $showFilterCategoryNews .= '<a href="'.URL.'tin-tuc/'.createSlug($name).'" class="btn btn-sm btn-outline-success me-1 '.matchValue('active',$id,$activeCate).'"><small>'.$name.'</small></a>';  
}
